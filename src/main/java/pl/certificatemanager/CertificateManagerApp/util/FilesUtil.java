        ...
        for (int i = 0; i < customerList.getLength(); i++) {
            ...
            if (customer.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (customerRepo.existsByEmail(elementCustomer.getElementsByTagName("email").item(0).getTextContent())) {
                    ...
                    String oldFirstName = oldCustomer.getFirstName();
                    String oldLastName = oldCustomer.getLastName();
                    String oldPhoneNumber = oldCustomer.getPhoneNumber();
                    String oldCity = oldCustomer.getCity();

                    if (!(oldFirstName.equals(elementCustomer.getElementsByTagName("firstName").item(0).getTextContent()))) {
                        responseMessage.setMessage("Customer with email " + elementCustomer.getElementsByTagName("email").item(0).getTextContent() + " is already associated with first name " + oldFirstName + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(elementCustomer.getElementsByTagName("email").item(0).getTextContent());

                    }

                    if (!(oldLastName.equals(elementCustomer.getElementsByTagName("lastName").item(0).getTextContent()))) {
                        responseMessage.setMessage("Customer with email " + elementCustomer.getElementsByTagName("email").item(0).getTextContent() + " is already associated with last name " + oldLastName + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(elementCustomer.getElementsByTagName("email").item(0).getTextContent());

                    }

                    if (!(oldPhoneNumber.equals(elementCustomer.getElementsByTagName("phoneNumber").item(0).getTextContent()))) {
                        responseMessage.setMessage("Customer with email " + elementCustomer.getElementsByTagName("email").item(0).getTextContent() + " is already associated with phone number " + oldPhoneNumber + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(elementCustomer.getElementsByTagName("email").item(0).getTextContent());

                    }

                    if (!(oldCity.equals(elementCustomer.getElementsByTagName("city").item(0).getTextContent()))) {
                        responseMessage.setMessage("Customer with email " + elementCustomer.getElementsByTagName("email").item(0).getTextContent() + " is already associated with city " + oldCity + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(elementCustomer.getElementsByTagName("email").item(0).getTextContent());
                    ...
                    if (customerRepo.existsByPhoneNumber(elementCustomer.getElementsByTagName("phoneNumber").item(0).getTextContent())) {
                        responseMessage.setMessage("Customer with phone number " + elementCustomer.getElementsByTagName("phoneNumber").item(0).getTextContent() + " is already saved in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(elementCustomer.getElementsByTagName("email").item(0).getTextContent());
                    }
        ...
        try {
            ...
            for (String[] row : allData) {
                if (!(customerRepo.existsByEmail(row[3]))) {
                    if (customerRepo.existsByPhoneNumber(row[2])) {
                        responseMessage.setMessage("Customer with phone number " + row[2] + " is already saved in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(row[3]);
                    }
                    ...
                    if (!(oldFirstName.equals(row[0]))) {
                        responseMessage.setMessage("Customer with email " + row[3] + " is already associated with first name " + oldFirstName + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(row[3]);

                    }

                    if (!(oldLastName.equals(row[1]))) {
                        responseMessage.setMessage("Customer with email " + row[3] + " is already associated with last name " + oldLastName + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(row[3]);

                    }

                    if (!(oldPhoneNumber.equals(row[2]))) {
                        responseMessage.setMessage("Customer with email " + row[3] + " is already associated with phone number " + oldPhoneNumber + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(row[3]);

                    }

                    if (!(oldCity.equals(row[4]))) {
                        responseMessage.setMessage("Customer with email " + row[3] + " is already associated with city " + oldCity + " in the database! Customers listed after weren't imported to the database due to the error.");
                        deleteFile(path);
                        throw new CustomerAlreadySavedException(row[3]);
